 ...
 * matrix. Application of the inverse of this filter (if the inverse exists) is
 * equivalent to solving a tridiagonal system of equations.
 * <p>
 * When applied to an input array x of length n, the filter computes elements of
 * an output y as follows:
 * <pre><code>
 * y[i] =            af*x[i] + b*x[i+1] ; for i = 0
 * y[i] = b*x[i-1] + ai*x[i] + b*x[i+1] ; for i = 1, 2, ..., n-2
 * y[i] = b*x[i-1] + al*x[i]            ; for i = n-1
 * </code></pre> To facilitate different assumptions about values off the ends
 * of arrays (boundary conditions), the coefficients <code>af</code> and
 * <code>al</code> for the first and last equations may differ from the
 * coefficient <code>ai</code> used in the interior equations. In a tridiagonal
 * matrix representation of this filter, the coefficient <code>af</code> is in
 * the upper left corner, and the coefficient <code>al</code> is in the lower
 * right corner.
 * <p>
 ...
 * <p>
 * This filter and its inverse (if that exists) may be applied in place. For all
 * methods, the input array <code>x</code> and output array <code>y</code> can
 * be the same array.
 * <p>
 * When applying an inverse filter, only a few simple checks are performed to
 * ensure that the inverse exists, that the tridiagonal matrix is not singular.
 * For example <code>|a|&ge;2|b|</code> is required, so that the matrix is
 * diagonally dominant.
 * <p>
...
public class SymmetricTridiagonalFilter {
    ...
    public void applyInverse(float[] x, float[] y) {
        ...
        else // Unlike Boisvert, we do not assume special values for u and v.
