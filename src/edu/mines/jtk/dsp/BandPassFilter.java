 ...
 * Fourier transforms. The result is equivalent to convolution with an ideal
 * symmetric (zero-phase) band-pass filter that has been smoothly tapered to
 * zero.
 * <p>
 * Filter parameters include lower and upper frequencies that define the pass
 * band, the width of the transition from pass band to stop bands, and the
 * maximum error for amplitude in both pass and stop bands.
 * <p>
 * For efficiency the Fourier transform of the filter may be cached for repeated
 * application to multiple input arrays. A cached transform can be reused while
 * the lengths of input and output arrays do not change. Because caching
 * consumes memory, it is disabled by default.
 *
...
public class BandPassFilter {
    ...
    /**
     * Sets the method used to extrapolate values beyond the ends of input
     * arrays.
     *
     ...
     * Caching consumes memory but improves performance by about 50% when the
     * same filter is applied repeatedly to arrays that have the same
     * dimensions.
     *
    ...
    /**
     * Gets the 1D array of coefficients for this filter. The origin of the
     * filter is at the center of the array.
     *
    ...
    /**
     * Gets the 2D array of coefficients for this filter. The origin of the
     * filter is at the center of the array.
     *
     * @return the array of filter coefficients.
     */
    ...
    /**
     * Gets the 3D array of coefficients for this filter. The origin of the
     * filter is at the center of the array.
     *
     * @return the array of filter coefficients.
     */
